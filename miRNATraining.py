...
data_path = 'train_testdata/positive_training_dataset.txt'
pseodu_data_path = 'train_testdata/positive_testing_dataset.txt'
# data_path = 'train_testdata/negative_training_dataset.txt'
# pseodu_data_path = 'train_testdata/negative_testing_dataset.txt'


def RNA_to_feature_list(str_rna, str_cod):
    # 接受两个字符串,第一个字符串是基因序列,第二个字符串是对应的二级结构,最终把两段序列转化为目标向量
    structure = re.findall(r'-?\d{1,2}.\d{1,2}', str_cod)[0]  # 利用正则表达式提取自由能
    rna, cod = '', ''
    ...
    for i in cod:
        if i == '(':
            counter += 1.0 / 2
    rna_dic = {')': '0', '(': '0', '.': '1', 'A': '00', 'C': '01', 'G': '10', 'U': '11'}
    counter_dic = {}
